...
season22_start_MMWR<- MMWRweek2Date(2022, 40, 1)  #Beginning of week 40 for 2022-2023 season
season23_start_MMWR<- MMWRweek2Date(2023, 40, 1) #Beginning of week 40 for 2023-2024 season
...
hosp_capacity<-beds_raw  %>% dplyr::select(state, date, inpatient_beds) %>% mutate(date=as.Date(date), state=tolower(state), year=year(date)) %>%
  mutate(season= case_when(date>=season22_start_MMWR & date<season23_start_MMWR ~ "2022-2023",
                           date>=season23_start_MMWR & date< season23_end ~ "2023-2024",
                           date>=season24_start & date< season24_end_MMWR ~ "2024-2025",
                           date>=season24_end_MMWR & date< season25_end_MMWR ~ "2025-2026",
                           TRUE ~ NA)) %>%
           drop_na(season) %>%
...
num_facilities<- HHS_actuals_facility_raw %>% select(hospital_pk, state, collection_week) %>% mutate(date=as.Date(collection_week), state=tolower(state)) %>%
  mutate(season= case_when(date>="2022-10-08" & date<"2023-10-07" ~ "2022-2023",
                           ...
                           date>="2024-10-05" & date< "2025-10-04" ~ "2024-2025",
                           TRUE ~ NA)) %>%
           drop_na(season) %>%
...
hosp_epi_df <- hosp_covid %>% 
  ...
  filter(geo_value %notin% c("gu", "vi", "as", "pr", "mp")) %>% #exclude territories
  left_join(demog %>% dplyr::select(pop, geo_value=location)) %>%
...
## NSSP ED visits
```{r}
ed_covid_us<- read_csv("data/covidcast-nssp-pct_ed_visits_covid-202239-to-202552_us.csv")
ed_covid_states<- read_csv("data/covidcast-nssp-pct_ed_visits_covid-202239-to-202552_states.csv")
ed_covid<-ed_covid_us %>% bind_rows(ed_covid_states) %>% dplyr::select(-`...1`) %>%
  mutate(year=as.numeric(str_sub(time_value, start = 1, end = 4)), week=as.numeric(str_sub(time_value, start = 5)), end_of_week_date=MMWRweek2Date(year, week, 7)) %>%
  dplyr::select(geo_value, time_value=end_of_week_date, value)

ed_flu_us<- read_csv("data/covidcast-nssp-pct_ed_visits_influenza-202239-to-202552_us.csv")
ed_flu_states<- read_csv("data/covidcast-nssp-pct_ed_visits_influenza-202239-to-202552_states.csv")
ed_flu<-ed_flu_us %>% bind_rows(ed_flu_states) %>% dplyr::select(-`...1`) %>%
  mutate(year=as.numeric(str_sub(time_value, start = 1, end = 4)), week=as.numeric(str_sub(time_value, start = 5)), end_of_week_date=MMWRweek2Date(year, week, 7)) %>%
  dplyr::select(geo_value, time_value=end_of_week_date, value)

write_csv(ed_covid, "data/ed_covid.csv")
write_csv(ed_flu, "data/ed_flu.csv")
...
COVIDHub_combo<- COVID_hub_files %>% 
  ...
  mutate(season= case_when(target_end_date>=season22_start & target_end_date<season23_start ~ "2022-2023",
                           target_end_date>=season23_start & target_end_date< season23_end ~ "2023-2024",
                           TRUE ~ NA)) %>%
           drop_na(season) %>%
...
FluSight23<-FluSight_ensemble23 %>% 
  ...
  filter(target_end_date>=season23_start & target_end_date< season23_end) %>%
  left_join(CDC_actuals, by=c("target_end_date", "location")) %>%
  filter(horizon >=0) %>% 
  # mutate(geo_value=replace_na(geo_value,"us"))%>%
