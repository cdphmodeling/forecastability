...
### Break out scoring by horizon= 0,1...,4
prelim_scores_horizon<-score(forecast_quantile) |> add_relative_skill(compare= "model", by=c("location", "disease", "season", "horizon"), baseline ="Baseline")

scores_horizon<- prelim_scores_horizon%>% summarise_scores(by=c("model", "target", "location", "season", "disease","horizon")) %>% 
  left_join(demog, by = "location") %>%
  left_join(hosp_capacity, by=c("location", "season")) %>%
  left_join(num_facilities, by=c("location", "season")) %>%
  left_join(seasonal_burden, by=c("location", "season", "disease")) %>%
...
write_csv(scores_horizon, "results/forecast_scores_horizon_MMWR.csv")
```
...
forecastability_horizon<- scores_horizon %>% left_join(combo %>% dplyr:: select(location, omega, season, disease), by= c("location", "season", "disease"))

write_csv(forecastability_horizon, "results/forecastability_horizon_results_MMWR.csv")
```
