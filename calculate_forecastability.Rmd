...
season22_start<-  as.Date("2022-10-02") # Beginning of week 40 for 2022-2023 season
#as.Date("2022-07-01")
season23_start<- as.Date("2023-10-01")  # Beginning of week 40 for 2023-2024 season
#as.Date("2023-07-01") 
season23_end<- as.Date("2024-04-30") #End of mandatory NHSN reporting period in 2024
season24_start<- as.Date("2024-11-01") #Restart of mandatory reporting
season24_end_MMWR<- MMWRweek2Date(2025, 39, 7) #End of 2024-2025 season
season25_end_MMWR<- MMWRweek2Date(2026, 39, 7) #End of 2025-2026 season
...
## State & National Scale: Evaluat forecastability
...
for(i in 1:length(unique(covid_hosp_full$geo_value))){
  diff_df$geo_value[i]<-unique(covid_hosp_full$geo_value)[i]
   ...
   diff_df$ndiff3_kpss[i]<- covid_hosp_full %>% 
       arrange(time_value, geo_value) %>%
          # mutate(weekly_hosp=log(weekly_hosp)) %>% 
    filter(time_value >=season22_start & time_value <= season23_end) %>%
     filter(time_value <= season23_end) %>%
     filter(geo_value==unique(covid_hosp_full$geo_value)[i]) %>% 
    pull(weekly_hosp) %>% ndiffs(test="kpss", alpha=confidence)
...
ts_complete<-covid_hosp_full  %>% 
  ...
  drop_na(diff) %>%
  pivot_wider(names_from=geo_value, values_from = diff) %>%
  filter(time_value>=season22_start+7 & time_value<=season23_end)
...
for(i in 1:length(unique(flu_hosp_full$geo_value))){
  diff_df_flu$geo_value[i]<-unique(flu_hosp_full$geo_value)[i]
   ...
   diff_df_flu$ndiff3_kpss[i]<- flu_hosp_full %>%
       arrange(time_value, geo_value) %>%
     # mutate(weekly_hosp=log(weekly_hosp)) %>% 
    filter(time_value >=season22_start & time_value < season23_end) %>% filter(geo_value==unique(flu_hosp_full$geo_value)[i]) %>% 
    pull(weekly_hosp) %>% ndiffs(test="kpss", alpha=confidence)
...
for(i in 1:length(unique(flu_ds$geo_value))){
  ...
  filter(.day=="Sat") %>% #down sample to weekly
    ...
    filter(time_value >=season22_start & time_value < season23_end) %>% filter(geo_value==unique(flu_ds$geo_value)[i]) %>% 
          mutate(.day= lubridate::wday(time_value, label = TRUE)) %>% 
  filter(.day=="Sat") %>% #down sample to weekly
    pull(weekly_hosp) %>% ndiffs(test="adf", alpha=confidence)
}
...
for(i in 1:length(unique(hosp_epi_df_county$geo_value))){
   ...
   diff_df_county$ndiff2_kpss[i]<- hosp_epi_df_county %>% 
    ...
    filter(time_value >=season22_start & time_value <= season23_end) %>% filter(geo_value==unique(hosp_epi_df_county$geo_value)[i]) %>% 
    pull(weekly_hosp) %>% ndiffs(test="kpss", alpha=confidence)
...
ts_county<-hosp_epi_df_county %>%
  filter(time_value <= season23_end, geo_value %notin% c("Calaveras", "Colusa", "Glenn", "Lassen", "Madera", "Mariposa", "Modoc", "Mono", "Plumas", "San Benito", "Santa Cruz",  "Siskiyou", "Sutter", "Trinity")) %>%
...
# ts_county<-hosp_epi_df_county %>%
#   filter(time_value <= season23_end, geo_value==i) %>%
...
for(i in 1:length(unique(hosp_epi_df_county_flu$geo_value))){
  ...
  filter(time_value >=season23_start & time_value <= season23_end) %>%
  filter(geo_value==unique(hosp_epi_df_county_flu$geo_value)[i]) %>% pull(weekly_hosp) %>% ndiffs(test="adf", alpha = 0.01)
...
ts_county_flu<-hosp_epi_df_county_flu %>% 
  filter(time_value <= season23_end, geo_value %notin% c("Amador", "Calaveras", "Colusa", "Del Norte", "Glenn", "Humboldt", "Inyo", "Lake", "Lassen", "Madera", "Mariposa", "Mendocino", "Modoc", "Mono", "Nevada", "Plumas", "San Benito", "Santa Cruz", "Siskiyou", "Sutter", "Trinity")) %>%
...
nssp_raw<-ed_covid%>%
  ...
  filter(geo_value!="wy") %>% #exclude wyoming which stops reporting mid summer 2024
  dplyr::select(geo_value, time_value, diff) %>%
  drop_na(diff) %>%
  pivot_wider(names_from=geo_value, values_from = diff) %>%
  filter(time_value>=season22_start+7)
...
spec22<- Omega(nssp_raw %>%
                 filter(time_value >= season22_start & time_value< season23_start) %>%
                 dplyr::select(-time_value), spectrum.control = sc, entropy.control = ec)
spec23<- Omega(nssp_raw %>%
                 filter(time_value>=season23_start & time_value< MMWRweek2Date(2024,39,7)) %>%
                 dplyr::select(-time_value), spectrum.control = sc, entropy.control = ec)
spec24<- Omega(nssp_raw %>%
                 filter(time_value>=MMWRweek2Date(2024,39,7) & time_value<season24_end_MMWR) %>%
                 dplyr::select(-time_value), spectrum.control = sc, entropy.control = ec)
spec25<- Omega(nssp_raw %>%
                 filter(time_value>=season24_end_MMWR & time_value<season25_end_MMWR) %>%
                 dplyr::select(-time_value), spectrum.control = sc, entropy.control = ec)
...
df23<- data.frame(as.list(spec23)) %>% pivot_longer(everything(), names_to="state", values_to="omega")  %>%
  ...
  left_join(state_census %>% dplyr::select(pop=POPESTIMATE2019, state=ABBR) %>% mutate(state=tolower(state)), by = "state") %>%
  mutate(season="2023-2024")
df24<- data.frame(as.list(spec24)) %>% pivot_longer(everything(), names_to="state", values_to="omega")  %>%
    mutate(state=case_when(state=="in."~"in",
                         TRUE~state)) %>%
  left_join(state_census %>% dplyr::select(pop=POPESTIMATE2019, state=ABBR) %>% mutate(state=tolower(state)), by = "state") %>%
  mutate(season="2024-2025")
df25<- data.frame(as.list(spec25)) %>% pivot_longer(everything(), names_to="state", values_to="omega")  %>%
    mutate(state=case_when(state=="in."~"in",
                         TRUE~state)) %>%
  left_join(state_census %>% dplyr::select(pop=POPESTIMATE2019, state=ABBR) %>% mutate(state=tolower(state)), by = "state") %>%
...
covid_ed_visits<- df22 %>% bind_rows( df23, df24)
covid_ed_visits %>% ggplot(aes(log(pop), omega)) + 
  geom_point() + 
  facet_wrap(season~.)+ 
...
spec23<- Omega(nssp_raw %>% 
                 ...
                 dplyr::select(-time_value), spectrum.control = sc, entropy.control = ec)
spec24<- Omega(nssp_raw %>% 
                 filter(time_value>=season24_start & time_value<season24_end_MMWR) %>% 
                 dplyr::select(-time_value, us), spectrum.control = sc, entropy.control = ec)
spec25<- Omega(nssp_raw %>%
                 filter(time_value>=season24_end_MMWR & time_value<season25_end_MMWR) %>%
                 dplyr::select(-time_value), spectrum.control = sc, entropy.control = ec)
...
flu_ed_visits<- df22 %>% bind_rows( df23, df24)
flu_ed_visits %>% ggplot(aes(log(pop), omega)) + 
  geom_point() + 
  facet_wrap(season~.)+ 
  geom_smooth(method="lm")+
  theme_classic()

ed_omega<- flu_ed_visits %>% mutate(disease="Influenza") %>% bind_rows(covid_ed_visits %>% mutate(disease="COVID-19"))

write_csv(ed_omega, "results/ed_omega.csv")
